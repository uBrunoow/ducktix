...
- Cada equipe deverá informar um domínio de informação ao qual a aplicação será desenvolvida (Exemplo: clínica veterinária, biblioteca, etc). NÃO PODERÃO HAVER REPETIÇÕES DE UM MESMO DOMÍNIO ENTRE EQUIPES. Ao escolher o seu, informe na planilha abaixo para que tal domínio seja atribuído a vc ou sua equipe. Prazo para me informar:  10/09 às 19:00 - após esta data alunos que não tiverem informados serão considerados a não entregar/apresentar
...
- Uma aplicação deverá ser desenvolvida que faça uso e manipulação do banco de dados relacional. Tal aplicação não necessita apresentar uma interface gráfica, portanto, se a equipe preferir poderá usar a interface em modo texto (REST não será aceito como interface final da aplicação). Não será exigida uma linguagem de programação específica, entretanto, a solução exemplo e suporte da professora será apenas na linguagem Java. Esta aplicação deverá prover interfaces e respectivo funcionamento para as seguintes operações:
...
2) Um repositório ou diretório na nuvem contendo: (a) arquivos de código-fonte da aplicação desenvolvida, (b) um arquivo de backup do banco de dados e (c) um arquivo de texto com instruções para a compilação e execução da aplicação. Forneça o link do repositório no documento de texto solicitado no item 1. Este link deve ficar disponível até o encerramento do semestre, deve ter visibilidade pública ou direcionada a professora e não pode sofrer alterações após a defesa do trabalho. Além disto, arquivos nele contidos não deverão estar compactados.

O trabalho deverá ser entregue pelo moodle até as 19h00 do dia 24/09. O trabalho será defendido também neste dia, conforme agendamento que será divulgado pela professora até um dia antes. Alunos que não comparecerem a defesa, e no horário marcado, zerarão o trabalho.
